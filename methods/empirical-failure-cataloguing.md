...
Step 1 probing, run against a Jira-shaped PM system, a Salesforce-shaped CRM and a Drive-shaped
file server, produced these six traps:
...
Encoded as six atomic rules, that catalogue was complete on class (a) and empty on class (b), and
it regressed at full scale because of it. `../skills/` is the pack that went through the whole
loop instead, Step 1 probing plus the trial-and-fix cycle in Step 3, and it is the one behind the
measured results. The gap between the two runs is the argument for the small-batch gate.
